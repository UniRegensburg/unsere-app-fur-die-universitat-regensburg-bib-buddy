...
public class TextNoteEditorFragment extends Fragment {
  ...
  public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
    ...
    if (getArguments() != null) {
      ...
      Cursor c = databaseHelper.getReadableDatabase()
          .query(DatabaseHelper.TABLE_NAME_NOTE, null, null, null, null, null, null);
      c.moveToLast();
      Long currentDate = new Date().getTime();

      //TODO: A note file has to be created at database first to get an autoincrement noteFileId
  ...
  public void onPause() {
    ...
    NoteDAO noteDao = new NoteDAO(databaseHelper);
    if (text.length() != 0) {
      noteDao.updateNote(noteId, String.valueOf(text), note.getType(), String.valueOf(text),
          note.getCreateDate(), modDate, note.getNoteFileId());
    } else {
