...
public class RichTextEditor extends AppCompatEditText implements TextWatcher {
  ...
  /**
   * This method is called to make a text format bold.
   *
   * @param valid Boolean to check if the selected text contains bold text already.
   */
  ...
  /**
   * This method is called to make a text format italic.
   *
   * @param valid Boolean to check if the selected text contains italic text already.
   */
    ...
    if (style == 0 || style == 1 || style == 2 || style == 3) {
      if (start == end) {
        if (start - 1 >= 0 && start + 1 <= this.getEditableText().length()) {
          ...
          return before.length <= 0 || after.length <= 0 || before[0].getStyle() != style
      ...
      for (int i = start; i < end; i++) {
        ...
        for (int j = 0; j < spanLength; j++) {
          ...
          span = spans[j];
          if (span.getStyle() == style) {
            builder.append(this.getEditableText().subSequence(j, j + 1).toString());
            break;
          }
        }
      }
  ...
  /**
   * This method is called to make a text format underlined.
   *
   * @param valid Boolean to check if the selected text contains underlined text already.
   */
  ...
  protected boolean containUnderline(int start, int end) {
    ...
    if (start - 1 >= 0 && start + 1 <= this.getEditableText().length()) {
      UnderlineSpan[] before =
          this.getEditableText().getSpans(start - 1, start, UnderlineSpan.class);
      UnderlineSpan[] after =
          this.getEditableText().getSpans(start, start + 1, UnderlineSpan.class);
  ...
  /**
   * This method is called to make a text format strikethrough.
   *
   * @param valid Boolean to check if the selected text contains strikethrough text already.
   */
  ...
  protected boolean containStrikethrough(int start, int end) {
    ...
    if (start - 1 >= 0 && start + 1 <= this.getEditableText().length()) {
      StrikethroughSpan[] before =
          this.getEditableText().getSpans(start - 1, start, StrikethroughSpan.class);
      StrikethroughSpan[] after =
          this.getEditableText().getSpans(start, start + 1, StrikethroughSpan.class);
      return before.length > 0 && after.length > 0;
    }
    return false;
  }
  ...
  /**
   * This method is called to set bullets at the start of each text line.
   *
   * @param valid Boolean to check if the selected text contains bullets already.
   */
  ...
  /**
   * This method is called to make a text format quote by making it italic and
   * setting a stripe at the start of each text line.
   *
   * @param valid Boolean to check if selected text contains a quote already.
   */
  ...
  /**
   * This method is called to redo the last user input action.
   */
  public void redo() {
    if (!this.redoValid()) {
      return;
  ...
  /**
   * This method is called to undo the last user input action.
   */
  public void undo() {
    if (!this.undoValid()) {
      return;
  ...
  /**
   * Check if there is a user interaction to be redone.
   *
   * @return returns right, if there is something to redo and false if there is none.
   */
  ...
  /**
   * This method is called to check if there is a user interaction to be undone.
   *
   * @return returns right, if there is something to undo and false if there is none.
   */
  ...
  /**
   * This method is called to check if the clicked ButtonView has already been clicked
   * and marked as "activated".
   *
   * @param format Text style type
   * @return True if format type is already applied and false if it is not.
   */
  ...
  /**
   * This method is called to save the text before each user input
   * to create a action history for undo- and redo-methods.
   *
   * @param text  CharSequence of the EditText-element before user input is applied.
   * @param start Char-position where the changes of the user input start
   * @param count Number of existing chars that are changed by the user input
   * @param after Length of the new replacing CharSequence of the user input
   */
  ...
  /**
   * This method is called to save the text after each user input
   * to create a action history for undo- and redo-methods.
   *
